...
def flatten_words(word_lists_one, word_lists_two, word_lists_three):
    # from tkinter import _flatten  # Linux中tkinter使用不便
    def _flatten(a):
        for each in a:
            if not isinstance(each, list):
                yield each
            else:
                yield from _flatten(each)

    test_word = list(_flatten(word_lists_one))
    test_tag = list(_flatten(word_lists_two))
    pred_tag = list(_flatten(word_lists_three))

    results = []
    for t_w, t_t, p_t in zip(test_word, test_tag, pred_tag):
        result = []
